...
try{
 ...
 const exported=await page.evaluate(async()=>{
  ...
  window.exportPhase='decode';const video=document.createElement('video');video.muted=true;video.playsInline=true;video.style.cssText='position:fixed;left:0;top:0;width:2px;height:2px;opacity:.01';document.body.append(video);video.src=URL.createObjectURL(out.blob);await new Promise((resolve,reject)=>{const timer=setTimeout(()=>reject(Error('Export decode timeout')),15000);video.onloadeddata=()=>{clearTimeout(timer);resolve();};video.onerror=()=>{clearTimeout(timer);reject(Error('Export decode failed'));};});
  window.exportPhase='present decoded frame';await new Promise((resolve,reject)=>{const timer=setTimeout(()=>reject(Error('No presented export frame')),15000);video.requestVideoFrameCallback(()=>{clearTimeout(timer);video.pause();resolve();});video.play().catch(e=>{clearTimeout(timer);reject(e);});});
  ...
  const result={bytes:out.blob.size,width:video.videoWidth,height:video.videoHeight,pixelSum:sum,format:out.blob.type,rendererMode:v.renderer.mode,frames:v.renderer.frames,recording,decodedFrame:c.toDataURL('image/png')};URL.revokeObjectURL(video.src);video.remove();v.sync.dirty=false;v.sync.dispose();window.exportPhase='complete';return result;
 });
 const videoBytes=Buffer.from(exported.recording.split(';base64,')[1],'base64');assert.equal(videoBytes.length,exported.bytes,'recording transfer preserves all encoded bytes');await writeFile('artifacts/export.webm',videoBytes);
 ...
 check('actual video recording and decode',exported.bytes>1000&&exported.width===320&&exported.height===180&&exported.pixelSum>2000000);
