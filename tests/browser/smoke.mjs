...
try{
 ...
 const exported=await page.evaluate(async()=>{
  ...
  const result={bytes:out.blob.size,width:video.videoWidth,height:video.videoHeight,pixelSum:sum,format:out.blob.type};URL.revokeObjectURL(video.src);v.sync.dirty=false;v.sync.dispose();return result;
 });
 check('actual video recording and decode',exported.bytes>1000&&exported.width===320&&exported.height===180&&exported.pixelSum>200000);report.export=exported;
